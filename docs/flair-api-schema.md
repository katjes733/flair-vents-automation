...
`GET /api/thermostats/{id}/remote-sensor` (**singular**) returns exactly one currently-active sensor — for this account, right now, that's the thermostat's own onboard sensor (`is-tstat: true`), not any room's remote SmartSensor.

**Correction, from a later live re-check — the claim above ("`ambient-temperature-c` reflects only the thermostat's own physical location") does not actually hold and should not be relied on.** Re-checked directly, room by room, on a live tick: the thermostat's own room ("Den Front," confirmed by the household directly — the physical unit reads 72.1°F/22.28°C on its own) has a `rooms.current-temperature-c` that does **not** match `thermostat-states.ambient-temperature-c` (72.7°F/22.61°C at check time) — even though `remote-sensor` (singular) still reports the active sensor as the thermostat's own onboard one, room "Den Front." Instead, at that moment, `ambient-temperature-c` exactly matched two *other* rooms' `current-temperature-c` ("Den back," "Martin Office," both 22.61°C).

**Follow-up correction — the "two rooms matched exactly" observation was a stale-data coincidence, not evidence either room lacks a real sensor.** A direct per-room check of `GET /api/rooms/{id}/remote-sensors` found every one of the five rooms genuinely has its own independent hardware: Martin Bedroom, Luke Bedroom, Den back (whose sensor is actually named "Extra Den" in the Ecobee app — a real naming mismatch worth knowing about, not a data bug), and Martin Office each have their own real `ecobee_ecobee3_remote_sensor` entry, and each one's `current-reading.temperature-c` matched its room's `current-temperature-c` exactly; Den Front has no room-scoped `remote-sensors` entry at all because its reading comes from the thermostat's own onboard sensor instead (the `is-tstat: true` entry, confirmed via its separate `room` relationship). **Confirmed, concretely, that Flair's own sensor data updates on a multi-minute cadence, matching this plan's own prior finding** (see the implementation plan's "Loop mechanism" section, "real-world sensor reporting latency") — two script runs 45 seconds apart returned byte-for-byte identical readings and identical `created-at` timestamps across all five rooms, meaning nothing had actually refreshed upstream in that window. A re-check a few minutes later than the original snapshot above showed Den back's reading had since diverged from Martin Office's (no longer matching), while `ambient-temperature-c` now matched Martin Office alone, exactly — consistent with the original "both matched" being two rooms momentarily sharing a stale cached value from before either had last updated, not a structural mirroring relationship. **Practical implication, independent of the original mystery**: a real reading in this account can be 10+ minutes stale between refreshes, and a value can appear to "jump" sharply between two of this app's own polls even though the underlying change was gradual — that's an upstream data-freshness characteristic to expect and design around (staleness detection already exists for exactly this — see the implementation plan's "Stale Sensor Reading Safeguard" section), not a bug in this app's own polling. **Still unresolved**: which sensor(s) actually feed `ambient-temperature-c` and by what rule (single active sensor vs. some averaged subset) — the `remote-sensor` (singular) field's claim continues to not visibly match which room's value `ambient-temperature-c` tracks. **Doesn't affect this plan's own correctness, deliberately**: Piece 1 (offset correction) was already designed to need nothing beyond the resolved `ambient-temperature-c` value itself, specifically so it wouldn't matter which sensor(s) actually feed it or how fresh they are — this correction only means the old inline claim about *why* that design choice was safe (a single, known sensor) was wrong; the design choice itself (not depending on knowing) is what actually makes it safe, and still holds.
...
| Active comfort-setting sensor group — present? | **Resolved (differently than assumed), then partly corrected.** There's no separate "sensor group" list to read or write; `thermostats.remote-sensor` (singular) claims to tell you which one sensor is active, and `thermostat-states.ambient-temperature-c` gives you the resolved comparison value directly — but a later live re-check found `remote-sensor`'s claimed room doesn't actually match which room's reading `ambient-temperature-c` agrees with (see the correction under [`remote-sensors`](#remote-sensors)). Doesn't affect this plan's design, which never depended on knowing the real answer. Writability untested (not needed either way). |
...
11. **The setpoint-push rounding granularity (`setpoint_push_rounding_c`, currently 0.5°C) is itself the wrong unit to round in, independent of whatever the real write test in item 5 finds.** This account's thermostat is Fahrenheit-native (per the household's own display preference) — rounding in fixed Celsius steps produces values that land on awkward, non-round Fahrenheit numbers (e.g. 71.6°F, 72.1°F) rather than the whole- or half-degree Fahrenheit increments a real thermostat call would normally use. Two related things worth fixing together, once a real write test confirms what Flair's API actually accepts: (a) round in the *display* unit that matches the account's own thermostat, not a fixed Celsius step, and (b) the tick decision's own `pushed_value`/`pushed_value_c` fields are currently the raw, unrounded `computeSetpointPush()` output — the rounding only happens inside `pushSetpoint()` at the actual write call, so the UI's "Thermostat call" display (`AirHandlerStatusCard.tsx`) shows a value more precise than anything that would ever really be sent, which is misleading during shadow-mode evaluation specifically. Deliberately not fixed now — flagged to revisit once items 5/`Not yet tested`'s structure-setpoint write test actually happens, since the correct rounding rule depends on what that test finds.
