...
// A safety net against two overlapping cycles for the SAME installation —
// distinct from BullMQ's own per-job lock (which only prevents two workers
// picking up the identical job instance; a repeatable job scheduler
// produces a NEW job instance every firing, so BullMQ's lock alone doesn't
// stop this cycle's job from still running when the next one fires). Also
// what makes triggerImmediateTick() safe to call concurrently with the
// scheduled job — both go through this same lock. Set well above any
 ...
 * queue.ts's registerInstallationTick); the fixed-id "queue-health-snapshot"
 * job fires once for the whole queue, independent of installation count.
 */
export async function tickProcessor(job: Job): Promise<void> {
  if (job.name === "queue-health-snapshot") {
    await processQueueHealthSnapshot();
    return;
  }
  await processInstallationTick(job.data.installationId as string);
}

/**
 * One installation's own full cycle, lock-guarded so it can never overlap
 * with another firing for the same installation — whether that's the next
 * scheduled job, or an explicit triggerImmediateTick() call arriving while
 * one is already in flight. Shared by both call sites for exactly that
 * reason.
 */
export async function processInstallationTick(
  installationId: string,
): Promise<void> {
  const acquired = await redis.set(
    `lock:tick:${installationId}`,
    "1",
    "PX",
    TICK_LOCK_TTL_MS,
    "NX",
  ...
  if (acquired !== "OK") {
    log.debug(
      ...
      "Tick already in flight for this installation — skipping this cycle",
    );
    return;
  }
  try {
    const installation = await getInstallationById(installationId);
    if (!installation) return; // deleted since the job was scheduled

    await maybeSeedStartupReconciliation(installation);
    ...
    await Promise.race([runTickForInstallation(installation), watchdog]);
    if (timedOut) {
      log.error(
        {
        ...
        "Control tick exceeded watchdog timeout — the next scheduled firing will still run regardless",
      );
    }
  } finally {
    await redis.del(`lock:tick:${installationId}`);
  }
}

/**
 * One installation's own full cycle — every active air handler on it,
 * sequentially, sharing a single FlairClient (and therefore a single
 * OAuth token / rate-limit budget) across all of them, per "Loop
 * mechanism": more air handlers under the same account is a within-tick
 * concern, not a horizontal-scaling one. Moved here from scheduler.ts
 * (formerly runHandlersForInstallation) once ticking moved to a BullMQ
 * job per installation instead of one process iterating every
 * installation in a single setTimeout loop.
 */
async function runTickForInstallation(
  ...
  const outageState = await client.getOutageState();
  ...
  const refreshFailure = await client.getTokenRefreshFailureState();
  if (refreshFailure) {
    await alerting.alertOnce({
...
// Not per-installation; the queue is one shared resource, and this is the
// concrete "should we add a worker" signal the plan names (queue lag, not
// CPU/memory).
async function processQueueHealthSnapshot(): Promise<void> {
  ...
  const workers = await tickQueue.getWorkers();
  const activeInstallations = await getActiveInstallations();
  logQueueHealthSnapshot(log, {
    waiting_count: counts.waiting ?? 0,
    active_count: counts.active ?? 0,
    delayed_count: counts.delayed ?? 0,
    failed_count: counts.failed ?? 0,
    connected_worker_count: workers.length,
